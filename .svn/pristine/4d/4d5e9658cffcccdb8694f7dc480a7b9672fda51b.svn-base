//MODIFY ANY CONFIGURATION DATA IN THE FOLLOWING OBJECT
function crmRemoteConfigClass() { 
    
		// change this.saccount to "devnissanusaext" for NISSAN DEV environments
		// change this.saccount to "nissanusaext" for NISSAN PRODUCTION environments
		// change this.saccount to "nissaninfiniticomdevinfinitiexternal" for INFINITI DEV environments
		// change this.saccount to "nissaninfiniticominfinitiexternal" for INFINITI PRODUCTION environments
		this.saccount = "nissaninfiniticomdevinfinitiexternal";
    
		// change this.domainSite to 'nissanusa.com' for NISSAN environments
		// change this.domainSite to 'infinitiusa.com' for INFINITI environments
		this.domainSite = 'infinitiusa.com';

		// ensure that your domain is listed here
    this.internalLinkFilters = "javascript:,eprize.net,intellipriceauto.com";
}

//DESIGNORY ONLY: MODIFY CODE BELOW THIS LINE
var remoteConfig = new crmRemoteConfigClass();

// ###	Primary tag firing function
function pingOmn () {
	omnCustEvents();
	
	//BLOCK: The following ONLY APPLIES to the remote version of this file
	s.hier1 = s.prop7 + '.' + s.prop1 + '.' + s.prop2 + '.' + s.prop3 + '.' + s.prop4 + '.' + s.prop5; 
    s.hier2 = s.prop7 + '.' + s.prop1 + '.' + s.prop2 + '.' + s.prop3 + '.' + s.prop4 + '.' + s.prop5; 
    s.hier3 = s.prop1 + '.' + s.prop2 + '.' + s.prop3 + '.' + s.prop4 + '.' + s.prop5;
	//END BLOCK
    
    s.t();
}

// ###	Primary tag firing function - not in use for remote implementation at this time
//function pingOmnLink () {
//	omnCustEvents();
//    s.tl();
//}
...
function omnCustEvents () {
	...
	if ( listContains(evts,'event4') || listContains(evts,'event8') || listContains(evts,'event11')) {
		s.events +=',event12';
...
//    return 'DFA#' + doubleclickAccountIdObj.getLocale() + ':v0=[["DFA-"+lis+"-"+lip+"-"+lastimp+"-"+lastimptime+"- "+lastclk+"-"+lastclktime]]';
//}
...
//var s_account=remoteConfig.environment + remoteConfig.accountSite + remoteConfig.locale; 
var s_account = remoteConfig.saccount
...
//s.linkInternalFilters="javascript:,nissanusa.com,infiniti.com"
s.linkInternalFilters = remoteConfig.internalLinkFilters;
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"
//s.fpcdomain=omnDomainSiteObj.getSite();
s.fpcdomain = remoteConfig.domainSite;
...
function s_doPlugins(s) {
    ...
    {
        s.prop38=(s.getQueryParam('Site','')=='' || /search.(infinitiusa|nissanusa).com/.test(location.host)) ? s.getQueryParam('CMP','') : (s.getQueryParam('Site','') + '.' + s.getQueryParam('Area','') + '.' + s.getQueryParam('Creative',''));
    } else {
...
var newArr = [
	...
	if(s.prop7 != '')
	{
		if(s.prop1.toLowerCase() == 'regional' )
		{
...
+"]&&(!s.isns||(s.apv<4||s.apv>=5))){s.d.write('<im'+'g class=\"omnReportImg\" name=\"'+imn+" // added class for custom style
...
+"'')+'://'+(s.ssl?s.sslfpcsubdomain:s.fpcsubdomain)+'.'+s.fpcdomain+'/b/ss/'+s.un+'/1/H" // fixed subdomain.domain error
...
+"ime()-b.getTime()<500)e=new Date}return ''}return '<im'+'g class=\"omnReportImg\" sr'+'c=" // added class for custom style
